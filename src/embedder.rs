...
fn with_query_embed_fast<T>(f: impl FnOnce() -> T) -> T {
...
fn apply_query_timeout_if_needed(client: LlmEmbedding) -> LlmEmbedding {
...
fn backend_ready_probe(backend: &LlmBackendKind) -> Result<(), AppError> {
    match backend {
        ...
        LlmBackendKind::OpenRouter => {
            if OPENROUTER_CLIENT.get().is_some() {
                ...
                Err(AppError::Embedding(
                    "openrouter probe: client not initialised (skip)".into(),
                ))
            }
        }
        LlmBackendKind::Codex => {
            ...
            if which::which(&bin).is_err() && which::which("codex").is_err() {
                return Err(AppError::Embedding(
                    "codex probe: binary not on PATH (skip)".into(),
                ));
            }
            ...
            match auth {
                ...
                _ => Err(AppError::Embedding(
                    "codex probe: auth.json missing (skip; use --llm-backend none or login)".into(),
                )),
            }
        }
        LlmBackendKind::Claude => {
            ...
            if which::which(&bin).is_err() && which::which("claude").is_err() {
                return Err(AppError::Embedding(
                    "claude probe: binary not on PATH (skip)".into(),
                ));
            }
            Ok(())
        }
        LlmBackendKind::Opencode => {
            ...
            if which::which(&bin).is_err() && which::which("opencode").is_err() {
                return Err(AppError::Embedding(
                    "opencode probe: binary not on PATH (skip)".into(),
                ));
            }
            Ok(())
        }
