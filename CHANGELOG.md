...
v1.2.6 declared GAP-SG-205 resolved. An end-to-end audit of that release measured the opposite, and this entry exists because the verdict was wrong rather than because something new broke.

- **GAP-SG-205 (reopened and closed)** — the target record was attached inside `base_meta`, which runs downstream of TWO short-circuits: `output::envelope::render` skips the whole layer when no shaping flag is set, and `agent_surface::apply` returns early when the surface is inert. Measured: `remember --db T` emitted no `agent_surface` block at all, while `remember --db T --max-items 50` emitted `db_path_source: "argv"`. The contract was therefore visible only to a caller who had already set an unrelated flag, and absent on the default path — the path every agent uses. A universal contract cannot hang off an optional block. The decision to enter the layer is now "a knob is set OR there is a target to report", `apply` gained an inert path that annotates without reshaping, and the record is merged at the one point both paths share. The failure envelope carries the target in its own struct, including on the hand-rolled fallback that exists precisely because serialization already failed — which is where knowing the target of a failed write matters most. Absence of the block now means one thing only: the process resolved no database, as with `config`, `completions` and `locale`.
- **GAP-SG-207 (closed)** — the other half of the rule: a verb with a side effect must NAME its target in the argv. v1.2.6 deferred this after estimating 47 argument structs. That estimate aimed at the wrong place. Every one of those declarations converges on `AppPaths::resolve`, the single funnel through which all 38 resolution sites pass, so the refusal fits in one place — the same DRY argument that put the agent-native layer at a single point. A `WritePolicy` installed beside the surface records whether the subcommand mutates and whether `--use-active` was passed; `resolve` refuses with exit 2 when a mutating verb ends up on the COMPILED DEFAULT with no dispensation. Which layer earns the refusal is the load-bearing decision. Read literally the rule would also reject an XDG target, and that reading is wrong for this product: `db.path` is a first-class key in the configuration registry, so an operator who ran `config set db.path` DID choose the database — once, instead of on every invocation — and refusing it would make the product's own configuration surface unusable. Measured: applying it to `xdg` turned 214 tests red, every one of them configuring `db.path` in a temporary XDG root. `TargetSource::Default` is the case the rule is actually about — nothing named the database anywhere, and the write lands in a compiled fallback the caller never mentioned. An `xdg` write is therefore permitted AND visible, rather than permitted and silent. `Commands::may_inherit_target` exempts `init`, whose purpose is creating the XDG database, and the host leaves `config`, `cache`, `slots` and `completions`, which call the same resolver to locate the MODELS directory and touch no database at all — without that arm the policy fired on `cache list`, a command that reads a cache and nothing else. `--use-active` is declared once, globally, and the envelope records it as `db_path_dispensation` only where it changed the outcome.
- **`discarded_flags`** — `rules-rust-cli-stdin-stdout-silent-discard` requires the failure JSON to name the arguments that were dropped. v1.2.6 folded them into the prose of the refusal, which forces a consumer to parse a sentence in one of two languages to learn which of its own flags were ignored. `AppError::Usage` now carries them as data and the error envelope emits the field, on the serde path and on the fallback alike.

### Instrumentation, constants and the record

- **`deep-research` declares its ceiling** — five of the six read commands called `universe::record`; `--max-results` truncates a fused ranking with no `record` beside it, so that command's narrowness stayed invisible. Now six of six.
- **The query ceiling reaches the inert path too** — instrumenting `deep-research` proved insufficient on its own: the ceiling was written by `base_meta`, which only the shaping path reaches, so `deep-research "x"` with no flag reported which database it had opened and said nothing about having cut the ranking to five. Half a contract, and the harder half to notice missing. The ceiling now rides beside the target for the same reason the target does — both are facts about the PROCESS rather than about the reshaping. Measured: `list --limit 3` with no knob reports `query_limit_kind: "pagination"` and `filter_scope: "page"`; `deep-research --max-results 5` reports `top-k`.
- **Four planned fields that never shipped** — `count_scope` names what `--count-only` counted, since the count runs AFTER `--max-items` and a cap of 10 over 39 matches answers `10`. `result_array_source` distinguishes a member the CLI declares as a result set from one the surface elected by fallback, which is the GAP-SG-203 misdirection made visible. `vocabulary_partial` reports that the suggestion vocabulary was sampled, so a short list is not read as "nothing resembles your key". `key_suggestions` carries the correction as data rather than inside a localized sentence.
- **Two orphan constants resolved by wiring, not deletion** — `K_LIST_DEFAULT_LIMIT` declared 100 while `list` carried a bare `50` in its body; a constant documenting a default the code does not use is worse than none, because it invites a reader to trust it. Renamed `K_LIST_TEXT_DEFAULT_LIMIT`, corrected to 50 and wired, which also removes the magic number. `K_RELATED_DEFAULT_LIMIT` was superseded by `related` borrowing `DEFAULT_K_RECALL`, tying this command's default to `recall`'s `-k` by accident; tuning one would have moved the other silently.
- **The target is now an argument, not a global read** — `apply` delegates to `apply_with_target`. The target lives in a process-wide `OnceLock`, correct for a one-shot binary and wrong for a test binary, where `paths` and `agent_surface` unit tests share one process: whether a target existed would have depended on which test ran first. Every test now states its own premise.
- **Documentation** — ADR-0067 records the query-ceiling / output-ceiling precedence contract and the choice of exit 2 over `EX_USAGE` 64, in both languages. The doc comment on `cap_secondary_arrays` claimed "envelope order"; `serde_json` is built here without `preserve_order`, so a `Map` is a `BTreeMap` and iteration is ascending by key. Roughly fourteen Portuguese comments outside `src/i18n` were translated, against the project's own English-code rule; the two in `memory_source.rs` stayed, because there the Portuguese IS the data being quoted.

### The gates that could not see themselves

The pre-publish gate for this release failed, and the three defects it exposed share one shape: a rule the project believes it enforces, routed somewhere that cannot enforce it.

- **GAP-SG-211** — 64 unresolved intra-doc links reached the gate, a class v1.1.1 had already driven to zero. Nothing regressed in the usual sense: `cargo check`, `cargo clippy -- -D warnings` and `cargo test` were green throughout, because the rustdoc book states these lints "are only available when running rustdoc, not rustc". `no_ci_workflows_gate` declares that `cargo test` is this repository's only automatic gate, so a rule needing a different tool was, by construction, enforced only when someone remembered a script outside the repo. The natural experiment sits in the same file: `#![deny(missing_docs)]` never regressed, and `missing_docs` is the one doc lint rustc can also see — same crate, same `deny`, opposite outcome. `[lints.rustdoc]` now denies `broken_intra_doc_links`, `private_intra_doc_links` and `invalid_html_tags` in the manifest, so `cargo doc` and `cargo test --doc` fail on their own instead of depending on `RUSTDOCFLAGS`. Two `#[allow(rustdoc::broken_intra_doc_links)]` sitting on `pub mod` declarations were blinding `memory_source` and `reaper` entirely, since the attribute propagates inward; both links had a clean full-path fix and both attributes are gone. `tests/fmt_gate.rs` brings `cargo fmt --all --check` inside the only gate that runs by itself.
- **GAP-SG-212** — the three `installed_binary_smoke_*` suites asserted that `~/.cargo/bin/sqlite-graphrag` matches the workspace version. That is a post-install property placed inside a pre-publish gate: the version is bumped before the gate runs and installing is only safe once it passes, so 26 tests were red by construction on every release. `include` ships `tests/**/*.rs`, so every crates.io consumer inherited them too. They now carry `#[ignore]` and libtest reports them honestly in the summary. The same helper hid a second defect: `skip_if_not_installed` called `std::process::exit(0)` when no binary was installed, and because libtest captures per-test output while a bare `process::exit` never flushes that capture, the explanatory message reached nobody while the suite reported success without running a single assertion — the same pattern `src/commands/optimize.rs` already forbids in `src/`, a rule never extended to `tests/`. It panics now, and `docs/TESTING.md` carries the post-install order in both languages.
- **GAP-SG-213** — `parse_k_range` bounds `-k` to `1..=4096` and had been wired to exactly two arguments, while all eight concurrency knobs carried a `clap::value_parser!(T).range(..)`. The parallelism surface was audited; the memory surface was not. `related --limit` was the sharp end: `Vec::with_capacity(limit)` and an `AHashSet` of the same capacity are allocated before any row can bound them, so an absurd value aborted the process on allocation instead of returning the exit code this crate reserves for memory pressure. Thirteen arguments across eight files now declare a `value_parser` behind one shared range check, with the four ceilings named in `src/constants/search.rs`. Measured and deliberately NOT treated as equals: the `export`, `pending`, `pending_embeddings` and `embedding` limits reach SQLite as a `LIMIT` clause, and large hop counts terminate at the graph diameter because both walks carry visited sets. `tests/numeric_bounds_gate.rs` fails, naming file and line, whenever a bounded numeric argument ships without a validator.

### The documentation said one thing and the repository did another

A root-level documentation audit ran after the pre-publish gate passed. It measured every document against the binary rather than against memory, and found the same shape a third time: a rule routed to a hand-maintained list instead of derived from the fact.

- **GAP-SG-214 (opened)** — four policy documents promised a CI that `tests/no_ci_workflows_gate.rs` actively forbids. `CONTRIBUTING` claimed `main` requires a passing pipeline for merge, that `Co-authored-by` is barred by CI, and named a `windows-build-check` job; `SECURITY` claimed CI runs `cargo audit` and `cargo deny` on every push; `CROSS_PLATFORM.pt-BR` claimed automated smoke tests on every pushed commit, on a line with no English twin. Measured: `.github/` holds four template files and no `workflows/` directory. The gate missed all of it because it only inspects lines that NAME a workflow file — `workflows/`, `ci.yml`, `release.yml` — so a prose sentence naming no file walks straight through, and because its `POLICY_DOCS` array lists three documents that never included `CONTRIBUTING` or `SECURITY`. The false sentences are corrected; hardening the gate is the declared residue, which is why the entry stays open.
- **Version seals converged on 1.2.7** — `llms.txt` and `llms-full.txt` carried a v1.2.5 title, `llms-full.txt` still declared v1.2.2 as current, `docs/DOCUMENTATION_FRAMEWORK.md` targeted crate 1.2.2, and `SECURITY` advertised v1.2.2 as the latest of the supported line. All four now name 1.2.7, with historical sections left untouched.
- **The numeric ranges reached the operator** — GAP-SG-213 changed the CLI contract and no operator-facing document said so. `README` and `docs/HOW_TO_USE` now state the four ceilings and that an out-of-range value is refused with exit 2 before the database is touched.
- **The target provenance reached the agent** — `db_path_source` and `db_path_resolved` existed only in `INTEGRATIONS`, absent from the two documents written for the agent that consumes the envelope. `docs/AGENTS` now carries them.
- **Twelve orphaned ADRs gained their Portuguese pair** — ADR-0007 through ADR-0018 shipped English-only against a project-wide bilingual policy, translated as immutable historical records rather than corrected.
- **Measured and found correct** — all 51 shipped verbs appear in the 17 inventory documents; all 63 XDG keys appear in both `README` files; every documented default matches `config doctor` exactly, in both languages; the 74 schema files on disk match the 74 the binary declares; all 61 ADRs are registered in `INDEX.md`; the `docs/COOKBOOK` heading asymmetry is allowlisted with a measured justification rather than being drift.

## [1.2.6] - 2026-08-10

### The agent-native surface stops answering about sets it never saw

Four measured defects, one shape: an impossible request was answered with an empty result and exit 0, which a caller cannot tell from "the data is not there". An agent reading that concludes the memory does not exist and writes a duplicate; an audit reading it closes a gap that is still open.

- **GAP-SG-201** — `--filter` judged the page the query returned, not the set the caller asked about. Measured: `--filter type=skill --count-only list` answers 39 over 1892 memories; the same request with `--limit 50` answered `{"count": 0}` with exit 0. The root cause was the absence of a precedence contract between the QUERY ceiling and the OUTPUT ceiling — `agent_surface::apply` receives the envelope already serialized, downstream of `LIMIT`, where an array of fifty is indistinguishable from a corpus of fifty. The command that applies a ceiling now declares it through `agent_surface::universe`, and a predicate over a truncated page is refused with exit 2 unless `--filter-scope page` declares the narrower reading. The refusal is scoped to ceilings that PAGE a countable universe: `list --limit`/`--offset` and `graph entities`, both of which run a `COUNT` beside the query. A `-k` in `hybrid-search` or `recall`, and `related --limit`, bound a ranking or a breadth-first walk — the top-k IS the answer rather than a truncation of one, so those are reported and never refused. Every envelope now carries `query_limited`, `query_limit`, `query_limit_source`, `query_limit_kind`, `universe_total` and `filter_scope`, so a narrow answer stops being invisible even where nothing is refused. `graph entities` is the case that fired without anyone doing anything wrong: its `--limit` defaults to 50, so `--filter entity_type=person graph entities` judged 50 of 15 622 entities on a command line that mentioned no limit at all.
- **GAP-SG-202** — a key that exists nowhere was indistinguishable from a value that happens to be absent. `--select body_length read` returned the envelope minus the field; `--filter chave_errada=x list` returned `count: 0` over 1892 memories; `--sort chave_errada` emitted no signal at all. All three exited 0, so a typo read as missing data. The documented contract promised that a malformed `--filter` fails fast with exit 2 "so a typo is never mistaken for an empty result set" — true of the SYNTAX and never of the key name, which is the common typo. `agent_surface::vocabulary` now resolves every requested key against the observed elements before any predicate runs, and `agent_surface::gate` refuses an unresolvable one with exit 2 and a `rapidfuzz` Jaro-Winkler suggestion: `nmae` answers "did you mean: name, namespace", `body_lenght` answers "did you mean: body, body_hash". Resolution scans EVERY element rather than a sample, because a wrong "absent" verdict would refuse a legitimate request; sampling is confined to building the suggestion list, which only runs after a key has already failed. Partial resolution stays successful and reports `key_resolution`, `resolved_keys` and `unresolved_keys`, so an agent projecting six fields over a heterogeneous set still gets a useful answer and learns which field was missing. `--allow-unknown-keys` restores the previous behaviour.
- **GAP-SG-203** — the surface FABRICATED a result array for envelopes that declare none, then applied the caller's predicate to it. `locate_result_array` falls back to the first array member when no well-known key matches, and `serde_json::Map` is a `BTreeMap` without `preserve_order`, so "first" means lowest by key. Measured: `--filter integrity_ok=false health` deleted all eight elements of `checks` — a collection the caller never named — while `integrity_ok: true` survived in the payload contradicting the very predicate, and the top-level `truncated` flag did not rise despite eight elements removed. That last part violated the invariant written in the module's own doc comment. The fallback is kept, because legitimate envelopes depend on it; what changed is that a key found only on the envelope is now refused with a message naming the array the predicate would have hit, so a wrong election is visible instead of destructive.
- **GAP-SG-204** — a knob declared against an envelope with no result array was accepted and ignored. Measured: `--sort chave --dedupe-by outra stats` exited 0 with both knobs listed in the record and neither applied. This is silent argument discard, which `rules-rust-cli-stdin-stdout-silent-discard` forbids in terms this release follows to the letter: an exit code must reflect whether ALL arguments were processed, and the refusal lists the discarded flags. `--select` is deliberately exempt: on an envelope with no result array it projects the envelope itself, which is a real effect.

### Explicit target designation

- **GAP-SG-205 (partial; the residue is tracked as its own entry in `gaps.md`)** — `AppPaths::resolve` resolves the database from three layers and only the first is the argv: `--db`, then the XDG key `db.path`, then the compiled default. A write verb reaching it without `--db` mutates a database the command line never named, and no envelope reported which one — so the wrong database could be written with no trace in the output. `TargetSource` now distinguishes `argv` from `xdg` and `default`, and the record carries `db_path_source` and `db_path_resolved`. Measured: with `--db` the envelope reports `argv` and the workspace database; without it, `default` and the XDG path. The other half of the rule — making the target REQUIRED in the argv with an explicit dispensation — is not applied here and is tracked openly: 47 argument structs declare `--db` independently, `init` legitimately creates the XDG database without it, and a blind rule would break commands whose ambient target is intentional. The remaining half is open and carries its own five-step plan.

### Errors, tests and structure

- **`AppError::Usage`** — no arm of `exit_code` returned 2, so a refusal discovered at output time had no code to map to. Exit 2 rather than `EX_USAGE` (64): `DUPLICATE_EXIT_CODE` was moved off 2 in v1.0.52 precisely to free it, `src/main.rs` already returns it for a rejected flag, and adding 64 now would give this binary two codes for one meaning. `apply` became fallible and `output::envelope::render` propagates it; the two `let _ = emit_json_compact(…)` in `src/commands/slots.rs` that swallowed every emission failure were removed.
- **The fence** — `apply` runs at OUTPUT time, after the handler already did its work, and `remember-batch` and `export` emit through `emit_json`. Refusing there for a command that changed durable state would report failure for an operation that succeeded, and a retried `remember` writes the memory twice. `Commands::mutates()` lists the read-only variants EXPLICITLY and answers `true` for everything else, so a subcommand added later and forgotten loses a diagnostic rather than duplicating data.
- **GAP-SG-206** — `cargo test --test gaps_caveat_gate` was RED on main before any of this. The blindness guard demanded more than thirty entries and more than thirty `- Status:` bullets; thirty was the size of the corpus the day it was written, so closing and pruning entries turned a healthy document into a failing build. A threshold cannot tell "the splitter broke" from "the backlog shrank", and only the first is a defect. The guard now compares headings against parsed entries and requires a status on each entry found, which holds at two entries, at two hundred and at none.
- **`truncated` could never rise on `list`** — `attach_meta` only wrote the flag when the member was ABSENT, and `ListResponse` declares `truncated` as a plain `bool`, so the member was always present. The module has always promised that removing data is never silent; that promise was quietly false on the most used command in the binary. The flag is now raised over an existing `false` and is monotonic, so a command that already truncated its own rows keeps saying so.
- **`src/agent_surface/tests.rs` split** — the file stood at 799 of the 800 physical lines `tests/file_size_ceiling_gate.rs` allows, so the next test written would have turned that gate red. Split by responsibility into `filters`, `projection`, `ordering`, `truncation`, `budget`, `aliases`, `surface_state`, `gate` and `vocabulary`, with the fixtures kept in the root every child shares. Unit coverage of the layer went from 47 tests to 69.

## [1.2.5] - 2026-08-04

### Read-path honesty (found while auditing the Grok CLI hooks, 2026-08-04)
- **GAP-SG-227** — `--fail-on-degraded` parsed, stored and was never read. Nothing in `src/` touched `cli.fail_on_degraded`; the decider `degradation_failure` sat in `src/query_embedding.rs` under thirty lines of doc comment and was called only by its own unit test. A degraded read returned FTS5-only results with exit 0 whether or not the flag was passed, while `gaps.md` had already promoted it to a standing rule for agents and both hook trees on the host taught it as protection. The root cause was not forgetfulness but shape: `resolve_query_embedding` returned the degradation PROSE and dropped the `reason_code`, and `degradation_failure` derives the error class from the code and never from the prose — so no caller could satisfy the contract. `QueryEmbedding` became a struct carrying `reason_code` (a struct rather than a five-tuple because `backend_invoked` and `reason_code` share a type and would swap silently), and `recall`, `hybrid-search` and `deep-research` now decide before running any query; `deep-research` decides before building its runtime so a refused search never pays for the fan-out. `--fallback-fts-only` stays exempt, because that degradation is the operator's own instruction.
- **GAP-SG-227 (guard)** — `tests/inert_flag_guard.rs` passed the whole time. Its predicate asked whether the field NAME appeared anywhere in `src/` outside the declaration, and `fail_on_degraded` appeared as the parameter of the uncalled decider and again inside its body: three pieces of real text, zero consumers. The predicate now asks who READS the value — `cli.<field>` outside the declaration, or `self.<field>` inside it, which is how every agent-surface knob reaches `AgentSurface`. `config_dir` and `cache_dir` joined `verbose` and `quiet` on the allowlist, all four being pre-parsed from raw argv before `Cli::parse` runs. A new assertion holds the line with the exact shape that got through.

### Enrich queue correctness (found while auditing the Claude Code hooks, 2026-08-04)
- **GAP-SG-224** — no dead-letter surface filtered by `namespace`, though the `queue` table has carried the column since the namespace-scoped queue migration and the pending path filters by it with a comment explaining why. `--list-dead`, `--list-skipped`, `--requeue-dead`, `--requeue-skipped`, `--prune-dead-orphans`, `--prune-dead-entity-orphans` and all five `dead_total` counts were unscoped while every one of their envelopes echoed `namespace` as if the result were scoped. `prune_dead_orphans` received `namespace` and used it ONLY to look up the memory in the main DB: a dead row belonging to namespace A was checked against namespace B's memories, found absent, and deleted as an orphan — silent cross-namespace data loss. `prune_dead_entity_orphans` was worse, consulting no DB at all and deleting every namespace's entity dead rows at once. Strict equality was chosen over `OR namespace = ''` because the migration tags legacy rows `operation = 'LegacyUnscoped'`, so the operation predicate already excluded them and the extra branch would be dead code reintroducing the leak.
- **GAP-SG-225** — the claim path has been protected by `with_busy_retry` since the bounded busy-retry landed; the WRITE-BACK was unprotected and its result discarded with `let _ =`, in the parallel drain and in the re-embed batch cycle. Under `--rest-concurrency 16` a SQLITE_BUSY on `mark_done` silently lost the completion: the row stayed in `processing`, the stale-claim sweep handed it back, and the provider was billed a second time for work already paid for. `drain_serial` already inspected these results, so the defence existed on one path and was missing on the other two. A shared `writeback` helper now applies the same retry budget and reports the loss; a lost write-back ends the invocation loudly instead of reporting a convergent drain.
- **GAP-SG-226** — `apply_sidecar_queue_pragmas` set `journal_mode` before `busy_timeout` while `apply_connection_pragmas`, in the same file, sets `busy_timeout` first. The sidecar is opened by every REST worker under fan-out, which is where SQLITE_BUSY appears, and neither function had any test of its effective `busy_timeout`. Order normalised and both now covered. Severity is stated rather than inflated: `docs.rs` records that a fresh rusqlite connection already carries a 5000 ms busy timeout, so the practical exposure was the configured value arriving late, not a zero timeout.

### Documentation audit of `docs/` (2026-08-04)
- **GAP-SG-216** — the gate built one round earlier for self-declared facts shipped green while six documents introduced **v1.2.2** as the current release, three releases after v1.2.5. `declared_current_versions` anchored on ``crate `version = "`` , the spelling of exactly one file; the other three spellings in the corpus never matched and were discarded before any comparison. The scanner now anchors on the currency PHRASES in both languages, requires a three-segment version so `schema is 13` is not read as a crate version, and exempts a version qualified by `since`/`desde`, which names where a behaviour started and stays true forever. The `README` and `AGENTS` banners stopped restating release contents: they hold the 1.2.x invariants and point at the CHANGELOG, so only the number changes per release — and the gate now charges for that number. The widened scanner then caught a SECOND hole in itself: it read the version AFTER the marker and not before, so `**v1.2.2 (current):**` in `INTEGRATIONS.md:3` and its Portuguese twin stayed invisible until `TRAILING_CURRENCY` covered the reverse order.
- **GAP-SG-217** — `docs/AGENTS.md` documented 17 of the 63 XDG keys and `docs/AGENTS.pt-BR.md` documented 15; no file under `docs/` carried the complete registry, and `HEADLESS_INVOCATION` carried 2. Both files now list all 63 keys with value kind and default, verified against `src/config/registry.rs` in both directions: zero missing, zero extra.
- **GAP-SG-218** — `HOW_TO_USE` required one of `claude` / `codex` / `opencode` installed and on `PATH`, told the reader to verify with `which claude || which codex`, taught how to unset API keys so a spawn would not abort, and pinned `cargo install --version 1.1.2` while claiming the check would print `1.2.5`'s predecessor `1.1.1`. All three backends were removed in v1.2.0. Rewritten to the OpenRouter key path. The pt-BR side also documented `--fallback-mode` and `--codex-model-validate`, which clap rejects and which the English side no longer had — dead instruction and parity gap on the same lines.
- **GAP-SG-220** — `docs/COOKBOOK.md` held 165 sections over 97 unique bodies: 68 were byte-identical duplicates spanning 2 269 of 5 494 lines. One title appeared twice with DIFFERENT content, and the shorter copy described the removed subprocess in the present tense. Deduplicated to 96 sections and 3 180 lines, keeping the correct long version of the collision; proven lossless by hashing section bodies before and after (97 unique to 96, the single loss deliberate). The parity allowlist justified itself with "the English file duplicates 68 sections verbatim" — a measurement that died with the fix. Rewritten around the structural fact instead: Portuguese carries 14 fewer headings over MORE bytes, so the two sides partition the same material differently.

### Documentation audit of `skills/` (2026-08-04)
- **GAP-SG-221** — both agent skills taught a surface the binary does not have. Four flags were declared global and are defined nowhere (`--extraction-backend`, `--strict-env-clear`, `--graceful-shutdown-secs`, `--openrouter-base-url`); `--embedding-backend` was documented as `auto|openrouter|llm` with `llm` described as a subprocess backend, while clap accepts exactly `auto` and `openrouter`; `config set spawn.skip_preflight=1` was taught as the emergency escape hatch for a key that never entered the registry; `--no-input` was credited with exit 65 and an MCP preflight with exit 16, neither of which exists, while the real exits 12 and 141 were absent. Seven flags that are per-subcommand were listed as global, and the write prefix both files taught placed `--llm-parallelism` before the verb — the position that exits 2. `tests/docs_command_coverage.rs` already reads both files, but its assertion is one-directional by construction: it requires every live command to be NAMED, never that every name is live. New gate `tests/skills_surface_gate.rs` compares every cited flag against the BUILT clap tree and every cited key against the registry, under two rules — known namespace, and any dotted token on a line that says `config set`/`config get`/`config unset`/`XDG`. The second rule exists because the first is structurally blind to an invented namespace: `spawn` is in no registry family, so the namespace test could never see it.
- **GAP-SG-222** — both files stated twice each that `schema` emits 75 NDJSON lines; it emits 74, and an agent told to expect 75 reads a complete catalogue as truncated. The Portuguese file contradicted itself ninety lines apart: one line recorded that `--resume` and `--retry-failed` were removed from `ingest`, another used `--resume` in its own ingest formula. The new gate anchors on the PHRASE the skill prints rather than a loose digit, because `74 NDJSON lines` is a promise about output the agent will count.
- **GAP-SG-223** — both files declared the same `name: sqlite-graphrag`, colliding with each other and matching neither directory. `--fail-on-degraded` appeared in neither, so nothing warned that a degraded read returns FTS-only results with exit 0; `ingest --enrich-after` appeared in neither, leaving the "never chain write and enrich" rule without its one sanctioned exception; nineteen further live flags were absent; and both listed 14 of the 63 XDG keys. Both also taught passing `--embedding-dim 1024` on every embed path — unnecessary, since 1024 is the default, and dangerous, since the flag overrides the dimensionality recorded in `schema_meta`. Rewritten against the measured surface: 51/51 commands and 63/63 keys in both languages, 19 H2 sections in the same order with identical bullet counts, and zero fabricated flags out of 177 real flags cited.

### Suite gate (the figure 1.2.4 omitted)
- Before: `cargo test --all-features` **exit 101**, 24 of 87 binaries reached, one failing guard, 1 893 assertions passing under `--no-fail-fast`.
- After: `cargo test --all-features` **exit 0** without `--no-fail-fast`; `cargo clippy --all-targets --all-features -- -D warnings` **exit 0**; `cargo test --doc` **exit 0**.

### Fixed
- **GAP-SG-189** — the GAP-SG-175 cascade recurred: `scan_page_size` reached `EnrichArgs` unclassified (GAP-SG-164 guard red), `scan/stream.rs` shipped four `redundant_closure`, and the `AppError::error_class` doctest had never compiled (`Internal` wraps `anyhow::Error`; `&str` does not satisfy it). All three arrived with GAP-SG-185, which was declared RESOLVED and verified.
- **GAP-SG-190** — `--no-input` documented exit 65 in `globals.rs` and eleven documents; no `AppError` arm returns 65. The refusal raises `Validation` → exit 1, now pinned by `tests/no_input_exit_contract.rs`.
- **GAP-SG-191** — `--max-items` stopped at the primary array: `graph --select id --max-items 2` returned 4 771 393 bytes (2 nodes, 59 066 edges). The cap now covers every array and reports `agent_surface.secondary_capped`. The `--max-output-bytes` stub reported the internally discounted budget (340 for a requested 400) and now echoes the requested value.
- **GAP-SG-192** — `init` filled `model` with the crate version while `init.schema.json` described it as the embedding model; it now carries the resolved model or `"none"`. XDG `embedding.model`, promised by `--embedding-model` since v1.0.93 and never read, is implemented.
- **GAP-SG-193** — the first `enrich --help` example was `--mode codex --codex-model gpt-5.4-mini`, which clap rejects; `health` told operators to install `claude`/`codex` while probing the OpenRouter key; `config` advertised `llm.fallback = codex,claude,none` against its own registry default; seven active schemas typed `backend_invoked` with six values the binary cannot emit.
- **GAP-SG-194** — `--max-output-bytes` cloned the whole envelope once per binary-search probe (~16 full-tree copies, >100 MB of churn on a 7.6 MB envelope). Replaced by a single forward scan over per-element serialized lengths; zero clones.
- **GAP-SG-197** — the documentation sat three releases behind the binary and nothing held it. No document was stamped for 1.2.3, 1.2.4 or 1.2.5; `MIGRATION.md` still announced "MIGRATING TO v1.2.2" with zero mentions of the three releases after it; `AGENTS.md` was stamped v1.2.1. The README pair documented 19 of 61 XDG keys, leaving 38 knobs invisible in every user-facing document, and `README.pt-BR.md` advertised `enrich.preserve_threshold`, `enrich.entity_connect.max_runtime_secs` and `llm.concurrency` — three keys that never existed, each answering exit 1. `AGENTS.md` never named six shipped commands and `INTEGRATIONS.md` never named thirteen. `HOW_TO_USE.md` taught a "CI Test Environment" requiring `claude` or `codex` on `PATH`, in a project that forbids CI and removed those backends in v1.2.0, while `DOCUMENTATION_FRAMEWORK.md` declared `ci.yml` and `release.yml` MANDATORY and miscounted all three of its own inventories. The dry-run envelope of `migrate` validated against no published schema variant, `agent-surface.schema.json` was absent from the schema index, and two of seventy-two schemas carried a `$id` under a domain that serves no schemas.

### Changed
- **GAP-SG-194** — `entity-descriptions` streams keyset pages via the new `keyset_for_each_selected`, which judges page fullness on rows SCANNED and `--limit` on items DELIVERED, so a page thinned by the Rust-side quality test is no longer read as end-of-table.
- **GAP-SG-193** — the `health` check formerly named `llm_cli` is now `embedding_key`; `health.schema.json` types `name` as a free string and no test pinned the old value.
- **GAP-SG-197** — two new documentation gates, because every earlier drift was mechanically detectable and nothing was checking. `tests/docs_xdg_coverage.rs` (6 assertions) fails when a registry key is missing from either README and when a README names a key the binary rejects; a separate assertion requires each allowlisted legacy mention to sit on a line that DENIES the key, so the allowlist cannot become a hole. `tests/docs_command_coverage.rs` (4 assertions) fails when a command from `--help` is absent from any of the 17 inventory documents; the matcher demands an invocation or a code span, because matching `export`, `list`, `read` and `related` by word boundary reported full coverage for a document that never showed the command. Suite: **92 binaries, 1 925 assertions, exit 0**.
- **GAP-SG-195** — `src/constants.rs` (960 lines) split into nine themed submodules re-exported by `src/constants/mod.rs`; every `crate::constants::X` path is unchanged and all 115 public constants are preserved. `tests/integration.rs` (2 485 lines) split into `integration_bootstrap`, `integration_memory_crud` and `integration_graph`, preserving all 70 tests (17 + 28 + 25), with the five shared helpers moved to `tests/common/`.
- **GAP-SG-199** — `resolved_busy_policy_defaults_match_constants` compared a RESOLVED value against a compiled constant while the resolver reads the host's real XDG config, so `config set db.busy_retries 12` — a documented key — turned the suite red without a line of code changing. GAP-SG-198 recorded this as a leftover and closed anyway. The unit test now asserts only the host-independent invariant; the default claim moved to `tests/busy_policy_hermetic.rs` under `common::isolated_env`.
- **GAP-SG-200** — `config set display.tz 0` stored with exit 0 and bricked every later invocation, including the `config unset display.tz` that would have undone it: `tz::init` propagated the validation error and `main` runs it before dispatching any subcommand. Every other XDG reader in the crate degrades; `current_tz()` in the same file already did. It now falls back to UTC with a warning naming both recovery commands.
- **GAP-SG-201** — `config set` validated the KEY and never the VALUE, so `embedding.dim nao-numero`, `log.format xml`, `i18n.lang klingon` and `llm.backend codex` all persisted with exit 0. `log.level` carried its own risk: `EnvFilter::new` DISCARDS an unparseable directive instead of failing, so broken syntax would silence logging outright while the command reported success (a bare token like `NIVEL_X` is NOT broken — the grammar reads it as a target name). `SettingKey` gained a `kind: ValueKind` field, all 63 keys are classified, and `set_setting` rejects a value outside the domain with a localized message naming what was expected.
- **GAP-SG-202** — `config get` answered `found:false` with exit 0 for a key that does not exist, while `config set` answered exit 1 with a did-you-mean for the same string, so a typo read as "exists, empty". The envelope now carries `known` and `suggestion`; the exit code stays 0 because scripts probe presence with this command.
- **GAP-SG-203** — `config set --help` published `enrich.preserve_threshold`, `enrich.entity_connect.max_runtime_secs` and `llm.concurrency`, three keys answering exit 1, and the GAP-SG-198 gate built to catch exactly this missed all three through four holes: it read only first-level subcommands (`config set` is second), anchored on two spellings the text did not use, `break`ed after the first candidate on a line, and hard-coded the nineteen namespaces its own leftover section admitted would go stale. All four closed; the namespaces now derive from the registry.
- **GAP-SG-204** — `enrich --llm-parallelism` documented itself as applying "only to the subprocess modes", which v1.2.0 removed — `EnrichMode` has one variant, so no such mode can be selected. The rationale moved from `///` to `//` because everything above the `#[arg]` renders into `--help`, where an operator needs the rule and not the release history. Drive-by on the same surface: `--llm-max-host-concurrency` rendered "optional XDG XDG" and described a ceiling over "LLM subprocesses" that no longer exist.
- **GAP-SG-205** — nine EN/pt-BR document pairs had drifted, and one was not cosmetic: `CROSS_PLATFORM.pt-BR.md` described an ACTIVE `windows-build-check` CI job in a project that forbids CI, because only the English side had been corrected. `HOW_TO_USE.pt-BR.md` still taught the "CI Test Environment" that GAP-SG-197 removed from the English file. Both fixed, along with seven other pairs; `SECURITY.md` and `docs/TESTING` also stopped naming `codex-models` and a deleted regression file as if they existed.
- **GAP-SG-206** — `DOCUMENTATION_FRAMEWORK.md` forbade `.github/workflows/` and, on the same page, kept a completion checklist marked `[x]` claiming `ci.yml` and `release.yml` had been created; GAP-SG-197 corrected only the prohibition half. Nothing prevented a workflow from being added either.
- **GAP-SG-207** — the C-toolchain exception was narrated and never bounded. Measured: `cc v1.2.60` is in the lockfile, FIVE packages declare it and only TWO compile C here (`libsqlite3-sys`, `ring`); `blake3`, `iana-time-zone-haiku` and `generator` declare it without compiling on any supported target. The alternatives were priced: `oxisqlite` 0.4.0 (no complete FTS5), `rustls-rustcrypto` 0.0.2-alpha. Neither is defensible for a memory store today, so the exception stands — now fenced.
- **GAP-SG-208** — four test files sat above the 800-line ceiling the project sets for itself (1393, 1311, 1002, 853), a leftover of GAP-SG-195 which met the ceiling in `src/` only. Split into sixteen files by command family with three new shared harnesses; all 145 test blocks preserved.
- **GAP-SG-209** — `config list --effective` promised "well-known defaults" and shipped a hand-written list of SEVEN pairs while the registry declares a default for about fifty keys. It was also a SECOND copy: `display.tz` was spelled `"UTC"` there and `Some("UTC")` in the registry, and `log.level` had already drifted, reporting `info` where the binary uses `warn`. It now reads the registry.
- **GAP-SG-210** — the 800-line ceiling never said WHAT it counted, so GAP-SG-195 closed it for `src/` using physical lines and GAP-SG-208 closed it for `tests/` using `tokei` code lines, which exclude comments and blanks. Under the stricter reading four test suites were still over: `installed_binary_smoke.rs` at 981, `schema_migration_integration.rs` at 923, `integration_graph.rs` at 922 and `cookbook_recipes.rs` at 907. The metric is now pinned to physical lines and enforced by `tests/file_size_ceiling_gate.rs`; the four were split into ten files with no test lost.
- **GAP-SG-211** — the documentation counted ITSELF and nothing recomputed the counts: both READMEs announced "61 keys" against a registry holding 63 and "64 schemas" against a directory holding 74, and both `llms*` manifests introduced the crate as "v1.2.2 (current)" while shipping 1.2.5. `CHANGELOG.pt-BR.md` listed `[1.2.4]` down among the 1.0.7x releases with the file preamble scattered around it; the two files held the same 128 releases, so a set comparison and the section-count guard both passed on a defect the reader can see. New `tests/docs_declared_facts_gate.rs` (11 tests) ties every declared total to its source and compares the two release orders POSITION BY POSITION, never as sets.
- **GAP-SG-212** — the documentation still taught the subsystem v1.2.0 removed. The README "Version Highlights" block carried 53 English and 49 Portuguese bullets describing `claude -p`, `codex exec`, `--bare`, ONNX and the daemon as the current surface, and `llms.txt` — the file an LLM agent reads FIRST — taught `--mode gliner|codex|claude-code`, the `--codex-*` flags, `SQLITE_GRAPHRAG_CODEX_BINARY` and `ingest --resume`, none of which exist: `ingest --mode` answers `[possible values: none]` and `enrich --mode` answers `[possible values: openrouter]`. The history block was replaced by a pointer to this file after verifying that none of the 50 commands and none of the 63 XDG keys lived only inside it, and that all 53 versions were already recorded here.
- **GAP-SG-215** — `init --help` opened with "Initialize database and download embedding model" and `cache clear-models` promised to force "re-download on next `init`", in a build that has downloaded nothing since v1.0.79 dropped the ONNX runtime; `cache list --json` answers `files: []` and `total_bytes: 0` because the directory never receives anything. The `--help` is the most authoritative document the product ships — it travels inside the binary and needs no network — and it was the only one left out of the documentation rounds. Residual declared: the `cache` command stays vestigial, and retiring it is a public-surface decision not taken here.

### Known limitations
- **Toolchain C via SQLite** — the rust-native mandate is incompatible with SQLite today: `rusqlite` `bundled` compiles SQLite in C via `cc`, `sqlx` depends on the same `libsqlite3-sys`, and the only rust-native implementation is alpha. FTS5, `load_extension`, `backup` and `functions` are all in use. Recorded in `gaps.md` as an OPEN entry with the evidence, not credited here as a fix — the entry closes when a stable rust-native SQLite with FTS5 exists.
- **GAP-SG-195** — `doc_contract_integration.rs`, `schema_contract_strict.rs` and `prd_compliance.rs` remain above 800 lines; each covers exactly one contract, so splitting them would fragment a coherent table.
- GAP-SG-144 Windows ACL remains type-checked only (no Windows host in this environment).


## [1.2.4] - 2026-08-03

### Sealed residual close-out (auditoria-execucao-02)
- **GAP-SG-185 technical close** — production enrich path uses `scan_operation_for_each` (page→enqueue) so the in-process candidate key buffer is O(page_size) for keyset-backed ops (memory-bindings, body-enrich, re-embed, domain/graph-audit/body-extract). Sidecar queue still stores one row per eligible item. Help text no longer claims full-corpus peak RSS is gone.
- **NC-3 / B7 class** — `llms.txt`, `llms-full.txt`, `llms.pt-BR.txt`, `SECURITY*`, `CONTRIBUTING*` no longer offer `OPENROUTER_API_KEY` as a live product channel; gate walk widened; denials document G-T-XDG-04 (`config add-key` / CLI flag only).
- **NC-2 honesty** — renamed `tests/ingest_claude_name_collision_regression.rs` → `tests/ingest_name_collision_regression.rs` (`IngestMode` is body-only; no `--mode claude-code` path).
- **GAP-SG-141 residual** — re-embed dry-run e2e on isolated TempDir (no monorepo job singleton).

### Fixed
- Overclaim that keyset SQL alone made production scan peak track the page (result `Vec` + enqueue retained all keys).
- Doc vector for a dead env var outside the previous operational walk list.

### Changed
- Enrich production orchestrator streams keyset pages into the sidecar queue; dry-run still materialises the full key list for preview.

### Known limitations
- GAP-SG-144 Windows ACL remains type-checked only (no Windows host in this environment).
- Relationship / entity-description / pair scanners still deliver one full page when not keyset-streamed.


## [1.2.3] - 2026-08-02

### Sealed 2026-08-03 (audit close-out)
- **GAP-SG-185 closed** — enrich scan keyset pagination (`--scan-page-size` / XDG `enrich.scan_page_size`, default 512); Linux RSS delta measured **16 KiB @ N=2k** vs **12 KiB @ N=20k** (ratio 0.75, sub-linear); parity test page_size=7 ≡ full.
- **NC-1** crate version bumped to **1.2.3** (was 1.2.2 with CHANGELOG already on 1.2.3).
- **NC-2** materialised `tests/ingest_name_collision_regression.rs` (force-merge name collision offline; renamed in 1.2.4 to `ingest_name_collision_regression.rs`).
- **NC-3** operational docs no longer offer `OPENROUTER_API_KEY` as a product channel; gate `tests/docs_env_channel_guard.rs`.
- **GAP-SG-141 residual** re-embed scan backlog measured on synthetic missing vectors (no network).

### Highlights
- **The output surface reshaped one array and let the aliases through (GAP-SG-155, GAP-SG-165)** — `locate_result_array` returned on the first known member, so every other array in the envelope escaped the pipeline untouched. Measured against 1.2.2: `list --filter memory_type=skill --limit 30` returned `items` with 2 entries and `memories` with 30, spanning seven types the filter had excluded; `--sort name` ordered `items`, left `memories` in insertion order, and still declared `"sort":"name"`. The envelope stated two contradictory truths about the same collection. Four aliases are now declared in `AGENT_SURFACE_ALIAS_ARRAYS` and dropped whenever a knob is set, listed under `agent_surface.aliases_removed`. A fourth alias, `related_memories` in `related`, had never been recorded anywhere.
- **`--max-output-bytes` returned ZERO data instead of a smaller payload (GAP-SG-166)** — the ceiling could only trim the array it addressed. With an alias present the untouched member kept the envelope over budget, so the surface emptied the canonical array, still did not fit, and replaced everything with a stub. Measured: `list --max-output-bytes 8000` produced 93 bytes and no records, while `hybrid-search` under the same ceiling produced 2 770 bytes **with** records. The flag that exists to fit a context window was making the command return nothing.
- **A passing test had encoded the defect as the specification** — `well_known_result_key_wins_over_an_earlier_array_member` used the real `recall` envelope and asserted that `direct_matches` stayed intact while `results` was capped. Fixing the behaviour required revoking that assertion, not just changing code.
- **The reshaped envelope did not validate against its own published schema (GAP-SG-160)** — the surface injects an `agent_surface` block, and every shipped schema declares `"additionalProperties": false`. JSON Schema draft-7 offers no way around that by extension, so the block is now a shared definition referenced by the per-command schemas, reachable as `schema --name agent-surface`.
- **Guards were scoped to the sample instead of the rule (GAP-SG-156, GAP-SG-157)** — the worker-count guard read its own file through `include_str!`, so `deep_research` kept a literal `.worker_threads(2)` while the guard stayed green; it now walks all of `src/`. The i18n guard banned two `AppError` variants out of nine, leaving eleven English messages reachable by a `pt-BR` operator.
- **A wall-clock assertion measured the host, not the code (GAP-SG-158)** — a 2 s ceiling wrapped around an external process spawn passed in 0.02 s alone and failed at 6.24 s under load. The property being tested is that the command aborts, and the observable signal for that is the exit code.
- **The ceiling still stubbed out when the second array was not an alias (GAP-SG-171)** — GAP-SG-166 closed the alias branch, but the top event was an OR gate, not an AND: `graph` pairs `nodes` with `edges`, a different collection that suppression must not touch and the trimmer could not reach. Measured: `graph --select id --max-output-bytes 4000` returned 81 bytes of stub. The budget now drains secondary arrays when emptying the canonical one still would not fit, preserving what the caller projected and sacrificing what it did not; the same command now returns 3 992 bytes carrying 343 real nodes. The stub survives for the genuinely irreducible envelope.
- **The byte ceiling measured one serialization and emitted another (GAP-SG-170)** — `budget::enforce` measures with `serde_json::to_string`, while `emit_json` rendered with `to_string_pretty` unconditionally. The cap was enforced on the compact form and violated on the indented one: a declared 8 000 produced 8 659 bytes, and the overshoot grew with the cap because indentation scales with content. Found while confirming GAP-SG-166, when the same envelope recompacted to 7 502 bytes — the trim had been correct all along, the unit had not. Output is now compact off a terminal, which both keeps the ceiling honest and satisfies the compact-JSON mandate; an interactive terminal still gets indented output.
- **One NDJSON contract for all three ingest pipelines (GAP-SG-148 item 5)** — `IngestFileEvent` / `IngestSummary` now serve the standard, codex and claude-code modes. Fields exclusive to one pipeline are `Option` with `skip_serializing_if`, so no consumer receives a null it must ignore. A **fourth** divergence surfaced during the unification and was closed with it: the phase frame existed twice, identical field for field except that one named the backend path `claude_path` and the other `codex_path`; it is now `IngestPhaseEvent`, with both members optional and skipped, leaving each mode's wire form unchanged byte for byte. The `indexed`/`failed`/`skipped`/`preview` vocabulary was likewise declared in two places and now lives once, beside the types that carry it. The entry was filed as two contracts, corrected to three, and closed at four — the count only settled when unification forced reading every serialized struct in all three modules instead of sampling.
- **`output_count` described the shaping stage, not the delivery (GAP-SG-172)** — the field is measured in `shape_items`, which runs before the byte ceiling exists; `finalize` re-attached `output_truncated` and `dropped` after the trim but never revisited the count. Measured: `list --limit 30 --max-output-bytes 8000` returned eleven elements beside `output_count: 30` and `dropped: 19`, which reads as a parser that lost nineteen rows rather than a ceiling doing its job; `deep-research --max-output-bytes 3000` reproduced it independently at 25 elements beside `output_count: 50`. The count is now re-measured off the surviving array. Safe for the reservation already made: the surviving length can only be smaller, so its decimal form never grows.
- **`super_hub_count` reported the sample cap, not the graph (GAP-SG-173)** — the count was `hubs.len()` over a query ending in `LIMIT 5`, so the field was structurally incapable of exceeding five while its documentation promised the number of entities above the degree threshold. A graph of 15 361 entities and 46 169 edges reported exactly five, next to a warning naming hubs of degree 1 452, 1 107 and 1 051. The count now runs `COUNT(*)` over the grouped subquery with no `LIMIT`; the sample keeps its cap and the warning states both figures. The whole class was swept rather than sampled — `non_normalized_count` proved correct, and the `len()` calls in `ingest_codex`, `deep_research` and `migrate` operate on in-memory vectors, leaving this as the only instance.
- **Four policy ceilings were literals inside SQL (GAP-SG-174)** — the release criterion checked `format!("LIMIT` and returned zero, but it was derived from one symptom and did not cover the category. Widening the sweep to `LIMIT [0-9]+` surfaced `LIMIT 50` in the `deep-research` edge fan-out, `HAVING deg > 50` and `LIMIT 5` in the health super-hub check, and `LIMIT 10` in the body-enrich prompt context — while the crate already had the `K_*` convention in `constants.rs` for exactly these ceilings, making it an internal inconsistency. All four are now named constants passed by bind. Structural literals are deliberately untouched: `LIMIT 1` for existence or top-of-list and `LIMIT 0` in the FTS module probe answer what shape the query has, not what value is reasonable.
- **A test shipped in this release aborted the harness, and the release gate did not see it (GAP-SG-175)** — `enrich_after_defaults` built its clap probe from an empty argv, but `operation` and `mode` are `required_unless_present_any`; clap answers a missing required argument by calling `exit(2)` on the spot, which ends the test process before the first assertion. The harness does not report a failed assertion in that case, it reports `error: test failed` with status 2, so none of the six tests had ever run. It stayed invisible because the closing check piped `cargo test` into `tail`, and a pipeline exits with the status of its last stage — cargo failed, `tail` succeeded, the shell reported success. Re-running with output to a file surfaced 1 366 passing tests **and** exit 2 at the same time; that contradiction is what led to the error. The probe now passes `--print-schema`, the cheapest member of the set that dispenses with both required arguments. The blast radius was larger than the file: `cargo test` stops at the first test binary that exits abnormally, and this one sorted 24th, so everything after it never launched — 23 suites and 1 366 assertions before the fix, 84 suites and 1 878 after. Sixty-one suites had never run. The number that looked like health was the symptom: it stopped growing because execution stopped. Process rule this pins: read the exit status before any pipe, or through `${PIPESTATUS[0]}`, and never let a pass count stand in for it.
- **The enrich wrote edges without reconciling `degree`, and fed on its own damage (GAP-SG-176)** — `upsert_relationship` writes the edge and nothing else, so each caller owns the `entities.degree` cache. `ingest::persist` and `remember::run` reconcile it; `enrich::postprocess` did not, and it is the path that runs most. The omission looped back: `entity-connect` picks hubs with `ORDER BY degree DESC` and islands with `degree = 0`, both read off that same cache, so each pass ranked hubs by a systematic undercount and offered as "islands" the very entities the previous pass had just connected. This was not a stale report, it was a corrupted algorithmic decision. Measured on a live graph: `graph recompute-degree` updated 6 218 of 15 370 entities, 40 % of the graph. An earlier audit had filed the symptom as documented behaviour under the note "degree is not automatic"; this round tested the note instead of accepting it and found six write paths that do maintain it.
- **`graph stats` mixed a live count with a cache in one envelope (GAP-SG-177)** — `node_count` and `edge_count` came from `COUNT(*)`, `avg_degree` was derived from those two, and `max_degree` alone read `MAX(entities.degree)`. Three fields measured the graph and one measured a snapshot: 856 here against 1 452 for the same graph in `health`. The envelope also carries an invariant needing no knowledge of the data — a maximum cannot fall below the mean of the same set — so a cache drifting to zero would ship `max_degree: 0` beside `avg_degree: 6.01`, an arithmetic impossibility the envelope declares about itself. `max_degree` is now measured over the edges, in both the namespaced and global branches.
- **Two policy values were inline in `llm_slots` (GAP-SG-178)** — `assumed_available_mb = 4096` and a 100 ms poll interval sat as literals on lines whose neighbours already read `LLM_WORKER_RSS_MB` and `MAX_CONCURRENT_CLI_INSTANCES` from `constants.rs`. Found by widening the GAP-SG-174 sweep past `LIMIT` to time, size and memory literals crate-wide. That widened sweep also confirms the category closed: literal URLs and filesystem paths appear only in tests and in a schema-identifying URI, the timezone layer uses UTC with an IANA name configurable through XDG, `SQLITE_GRAPHRAG_*` is never read by `env::var` anywhere in the crate, `openssl` appears in neither `Cargo.toml` nor the code, and `.github/` holds issue and pull-request templates with no workflow at all.
- Schema remains **v16**. No main-DB migration.

### Added
- `AGENT_SURFACE_ALIAS_ARRAYS`: the four derived arrays (`items`→`memories`, `nodes`→`entities`, `results`→`direct_matches`/`graph_matches`/`related_memories`) suppressed once a knob is set.
- `agent_surface.aliases_removed`: names the members dropped, so suppression is never silent.
- `docs/schemas/agent-surface.schema.json`, referenced by the command schemas and listed by `schema`.
- `tests/docs_consistency.rs`: fails when the newest CHANGELOG section credits a gap that `gaps.md` still reports as outstanding. This is the executor GAP-SG-161 asked for; it is a test because this project forbids CI by design.
- `tests/ingest_ndjson_contract.rs`, `tests/enrich_after_defaults.rs` and `tests/ingest_name_collision_regression.rs` (was `ingest_claude_*` until 1.2.4).
- `scan/sql.rs`: shared `LIMIT`/placeholder builders, so no scanner pastes a value into SQL text.

### Fixed
- The agent-native flags covered a single array per envelope; `list`, `recall`, `related` and `graph` leaked the rest (GAP-SG-155, GAP-SG-165).
- `--max-output-bytes` collapsed into a data-free stub whenever an alias sat outside the trimmable array (GAP-SG-166).
- `graph --format json` never reached the surface at all: it serialised with `to_string_pretty` and emitted through `emit_text`, so none of the eight flags applied. NDJSON streaming keeps its deliberate bypass (GAP-SG-169).
- `deep_research` built a second Tokio runtime pinned to two workers (GAP-SG-156).
- Eleven `AppError` messages were built with inline `format!` and reached `pt-BR` operators in English; `remember_batch` duplicated `errors_msg::duplicate_memory` with divergent wording (GAP-SG-157).
- `LIMIT` was interpolated into SQL text at six sites under `enrich/scan/` and two under `quality_sample`; the cap is now bound like any other parameter, using `LIMIT -1` as the unlimited sentinel (GAP-SG-167).
- `docs/schemas/README.md` claimed `--mode codex` reused the claude-code NDJSON format, untrue since 1.2.2 (GAP-SG-168).
- `docs/TESTING.md` listed unit tests in `filter.rs`, `shape.rs` and `budget.rs`; all 29 live in `tests.rs`.

### Changed
- `ingest --mode codex` and `--mode claude-code` emit the same event and summary types as the standard pipeline. Token totals, previously codex-only fields with different names, are now optional members of the shared contract.
- `embed_passages_parallel_with_embedding_choice` is `#[deprecated]` in favour of `embed_passages_parallel_shared`; it clones the whole corpus and had no in-crate caller left. The re-exports carry a scoped `#[allow(deprecated)]` so the warning reaches consumers rather than us.
- `graph --format json` emits compact JSON through the shared path instead of pretty-printed text.

### Known limitations
- GAP-SG-159 is closed as **not applicable**: it asked for a CI workflow to run the cross-target type-check, and this project forbids CI, CD and pipeline YAML. The Windows type-check stays a manual release step.
- The Windows ACL of GAP-SG-144 remains type-checked only; no Windows host is available to execute it.
- `re-embed` batching could not be measured on a live queue: this database reports full vector coverage, so the scan finds nothing to claim.

## [1.2.2] - 2026-08-02

### Highlights
- **Re-embed claims in batches — 32x fewer requests (GAP-SG-141 B1)** — the enrich queue modelled one row as one remote call, so a 36 343-item backfill issued 36 343 requests. `dequeue_batch_pending` claims up to `enrich.reembed_claim_batch` rows (default 32) in one atomic `UPDATE ... RETURNING`, resolves them without touching the network, and embeds the whole batch in a single call. A wiremock test proves 64 items now cost **2** requests instead of 64.
- **The CLI compiled on Linux only (GAP-SG-149)** — `spawn_opencode` has two mutually exclusive `#[cfg]` definitions and the doc comment sat above the Linux one. With `#![deny(missing_docs)]` active, the non-Linux branch was a **compile error** on Windows *and* macOS. No local gate could see it: on Linux that branch is never compiled. **Caveat:** the Windows ACL hardening of GAP-SG-144 is now type-checked (via the `x86_64-pc-windows-gnu` target) but has never been executed on a Windows host.
- **`ingest --mode claude-code` duplicated chunk rows on every re-run (GAP-SG-150)** — that path inserted chunk slices and never deleted the previous set. `memory_chunks` DOES carry `UNIQUE(memory_id, chunk_idx)`, so the re-insert collided at chunk 0, `insert_chunk_slices` returned `Err`, and the caller only emitted a `tracing::warn!`. The body was updated while the chunk index **stayed frozen on the first revision** — chunk retrieval scored passages from a body the database had already replaced. A body that shrank below the split threshold also left orphan slices behind. **Existing databases keep the stale index**; re-ingest to refresh it.
- **Agent-native output surface (GAP-SG-142)** — eight global flags let the binary shape its own payload: `--select`/`--fields`, `--filter`, `--max-items`, `--sort`, `--dedupe-by`, `--count-only`, `--truncate-content`, `--max-output-bytes`. Applied at a single emission point, so every subcommand gains them at once. Failure envelopes are never filtered: `error: true` / `ok: false` always reaches the caller.
- **All 74 shipped JSON Schemas are now reachable (GAP-SG-151)** — `schema` lists every contract as `{"id", "invoke"}` and `schema --name <ID>` emits the document. Previously 70 of the 74 shipped in the tarball with no verb to read them.
- **Embedding runtime and timeout stopped being hardcoded (GAP-SG-141 B2/B3)** — the Tokio runtime derives its worker count from `available_parallelism` instead of a literal `2`, and the embedding deadline resolves through `--openrouter-timeout` > XDG `embedding.timeout_secs` > named constant.
- Schema remains **v16** (no main-DB migration). Queue and ingest behaviour only.

### Added
- Global flags `--select` (alias `--fields`), `--filter`, `--max-items`, `--sort`, `--dedupe-by`, `--count-only`, `--truncate-content`, `--max-output-bytes`.
- Global flag `--no-input`: makes the existing TTY guard declarable — any path that would read stdin fails before attempting, even when a pipe is present.
- Subcommand `schema` (catalogue) and `schema --name <ID>` (document), matching the convention of the sibling CLIs in this toolchain.
- Windows DACL hardening for the config file carrying the OpenRouter key, the counterpart of the `0o600` applied under `cfg(unix)` (GAP-SG-144). **Caveat:** type-checked by cross-compilation, never executed on a Windows host.
- XDG keys `enrich.reembed_claim_batch`, `parallelism.embed_runtime_threads`, `enrich.rate_limit_deadline_secs` and the stdin read deadline; plus registration of five keys that were read at runtime yet rejected by `config set`.
- `history` now works for memories written by `ingest --mode claude-code`: that path records a version on update, as the standard pipeline always did.

### Fixed
- `ingest --mode claude-code` left the chunk index frozen on the first revision: the re-insert collided with `UNIQUE(memory_id, chunk_idx)` and the resulting `Err` was only warned about, so the body advanced while its slices did not (GAP-SG-150).
- The crate did not compile for any non-Linux target (GAP-SG-149).
- `ingest --enrich-after` pinned `EnrichMode::ClaudeCode` and `openrouter_timeout: Some(300)`, so the automatic pass ran on a different backend and half the documented 600 s budget, shadowing XDG (GAP-SG-149).
- `--force-merge` was a silent no-op in claude-code mode: `upsert_memory` merged unconditionally, so two files deriving the same name overwrote each other without error, warning or signal (GAP-SG-148).
- `--llm-parallelism` is documented as inert under `--mode openrouter`, where `--rest-concurrency` governs fan-out, and now emits a warning when passed explicitly (GAP-SG-141 B7).
- Ten integration tests had been asserting contracts retired in v1.2.0 and v1.1.8; each now verifies the same question against the current contract, with the originating gap cited in a comment.
- `prd_namespace_flag_sets_memory_namespace` carried an unquoted SQL string literal, which SQLite read as a column comparison.

### Changed
- **Breaking, NDJSON:** `ingest --mode claude-code` now emits `IngestFileEvent` and `IngestSummary`, the same types as the standard pipeline. Per-file `status` changes from `done` to `indexed`; the summary reports `files_total` / `files_succeeded` / `files_failed` / `files_skipped` instead of `completed`. Events now go through `emit_json_compact`, which propagates I/O errors rather than swallowing them. A single parser now consumes both modes.
- `ingest --mode claude-code` embeds through the shared batch path instead of one call per chunk (GAP-SG-148 item 4).
- Time policy that was written as literals at the point of use is now named constants, with XDG keys where the value is operational policy. `Duration::from_secs(3600)` appeared identically in three files; the `base + 15s per item` computation was duplicated verbatim in two.
- Modules split by responsibility rather than by size: `config`, `embedding_api`, the enrich extraction operations, and both ingest pipelines. The `_a` / `_b` / `_c` suffixes are gone.

### Known limitations
- A batch re-embed failure returns every row in the claim to `pending`; the consumed attempt stays, since each item was really tried. The circuit breaker records **one** failure, because there was one remote call. A dimension mismatch fails the whole HTTP chunk and lands in this same policy.
- The Windows ACL path is verified by cross-compilation only.

## [1.2.1] - 2026-07-31

### Highlights
- **Enrich queue namespace isolation (claim + count + resume)** — `dequeue_next_pending`, `count_eligible_pending`, `--resume` / `--retry-failed` all require `operation` **and** `namespace`. An enrich drain for `ai-sdd` no longer claims or counts `global` / empty-ns rows.
- **`--until-empty` counts only this op+namespace** — previously counted *all* pending rows across operations, so alien ReEmbed zombies kept EntityDescriptions spinning until max-runtime with `completed=0`.
...
- **Re-embed eligibility uses BLOB length, not `dim` column alone** — scan/predicates select CORRUPT / META_AHEAD rows (`dim=1024` with a 384-d BLOB still eligible).
- **Enqueue validates re-embed keys** — `entity:{name}` strips the prefix for entity lookup; bare names still work; missing entities rejected. Chunk keys validate `chunk_id` exists in a non-deleted memory of the target namespace.
- **CAPA-D low-quality markers** — bare `%configuration file%` removed; compound markers only (e.g. `is a configuration file`) so legitimate domain prose (clippy.toml lint config) is not force-redescribe fodder.
- Schema remains **v16** (no main-DB migration). Sidecar queue behaviour only.
- Note: an older historical section also titled `[1.2.1] - 2026-04-18` (MSRV pin) remains deeper in this file for archaeology; **crate 1.2.1 published from this release is the July 2026 enrich-queue CAPA seal**.

### Fixed
- `enqueue_candidate` rejected every `entity:…` re-embed key (`entity not found in namespace`) → `completed: 0` on ~14k entity rows.
- Claim without namespace filter → cross-namespace HardFailure + circuit breaker.
- Chunk enqueue trusted the scanner alone → invalid / cross-ns chunks dead-lettered.
- `--force-redescribe` could not re-process rows already `skipped`/`done` in the sidecar.
- ReEmbed left pending forever when the live BLOB already matched the target dim.
- False-positive force-redescribe on legitimate "configuration file" descriptions.

### Added
- Regression: `enqueue_candidate_accepts_entity_prefixed_reembed_key` (accepts `entity:ownership` and bare `ownership`; rejects missing entity).
- Regression: `dequeue_next_pending_isolates_by_namespace`.
- Queue unit suite: 38 tests OK.

### Docs / packaging
- Product docs, monographs, skills, and `llms*` retargeted to **crate 1.2.1**.
- Full CLI command inventory retained/updated in operator-facing monographs.
...
- Bug 5 — `link` no longer treats pure-numeric strings as entity names under `--create-missing`. New `--from-id` / `--to-id` resolve by ID; `validate_entity_name` rejects digit-only names so ghost entities cannot be created.
...
- `graph traverse` notes: short-nickname behaviour and `--fuzzy`.
- `gaps.md` status flipped from pending → fixed for Bugs 1–5.
- `Cargo.toml` description binary size refreshed to ~19 MiB (matches the release binary; restores `binary_size_documented_regression` invariant).

## [1.1.04] - 2026-07-08

Closes the two structural gaps tracked in `gaps.md` after v1.1.03: the `deep-research` nested-Tokio-runtime panic (GAP-001) and the non-convergent `entity-connect` enrichment (GAP-002). The schema advances from version 15 to 16 with a numbered migration. Note: the official release name is v1.1.04; the crate manifest carries `version = "1.1.4"` because the SemVer parser rejects a leading zero in the patch component.

### Fixed
- GAP-001 — `deep-research` no longer panics with "Cannot start a runtime from within a runtime". The sync entry point now computes per-sub-query embeddings BEFORE building its dedicated Tokio runtime (new `compute_sub_embeddings` helper in `deep_research.rs`), and the three OpenRouter embedding paths in `embedder.rs` (single, serial batch, JoinSet fan-out) adopt the canonical `Handle::try_current` + `block_in_place` reentry pattern already used by the batch path — eliminating nested-runtime panics for any future caller that runs inside an existing runtime. The same guard is applied to `ingest_opencode`.
- GAP-002 — `entity-connect` now converges. A new `entity_connect_seen` table (migration V016) records the LLM verdict (`related`/`none`) for each evaluated pair, the `scan_isolated_entity_pairs` scanner excludes already-evaluated pairs and prioritises hub entities, `count_operation_backlog` reports a real O(n) backlog proxy (degree-0 entities with NER bindings) instead of a hard-coded zero, and `call_entity_connect` persists the verdict on both the `related` and `none` branches. `--until-empty` now reaches `eligible_remaining == 0` instead of re-evaluating the same rejected pairs forever.

### Added
- Migration V016 — `entity_connect_seen(source_id, target_id, namespace, verdict, relation, evaluated_at)` with composite primary key, dual `ON DELETE CASCADE` foreign keys to `entities(id)`, a `verdict` check constraint, and a namespace index. `CURRENT_SCHEMA_VERSION` advances from 15 to 16.

### Docs
- The `entity-connect` enrich operation is promoted from "scan-only (does not persist)" to "fully-implemented" in the skill reference, reflecting the convergent verdict-tracking pipeline.
- The `Cargo.toml` description re-includes the binary size (~16 MiB), restoring the `binary_size_documented_regression` invariant that drifted during the v1.1.03 description refresh.
## [1.1.03] - 2026-07-07
...
### Added
- 10 new subcommands for the LLM pipeline: `pending list`, `pending show`, `pending cleanup`, `embedding status`, `embedding list`, `embedding abandon`, `pending-embeddings list`, `pending-embeddings process`, `slots status`, `slots release`.
- The `pending` family (V014 — `pending_memories` table) provides a 3-stage checkpoint for the `remember` pipeline. The checkpointer survives a crash; on restart the operator can use `pending list` to inspect the queue and `pending show <id>` for a single entry.
- The `embedding` family exposes the LLM embedding queue, with `--filter-status queued|processing|done|failed|skipped` and `--llm-backend codex,claude,none` for the retry-fallback pipeline.
- The `slots` family exposes the host-wide semaphore: `slots status` reports `max_concurrency`, `acquired`, `waiting`, `held_by_pid[]`; `slots release --slot-id N --yes` reaps orphaned slots.
- 6 new global flags: `--max-concurrency <N>`, `--wait-lock <SECONDS>`, `--llm-parallelism <N>` (default 4, clamp [1, 32]), `--ingest-parallelism <N>`, `--graceful-shutdown-secs <N>`, `--skip-embedding-on-failure` (valid only with `--llm-backend …,none`).
- Lock contention via `fs4 = 0.9` with `fcntl(F_SETLK)` on Unix and `LockFileEx` on Windows (ADR-0039).

### ADRs
- ADR-0036 (`pending_memories_staging.md`)
- ADR-0037 (`shutdown_json_envelope.md` — exit code 19)
- ADR-0038 (`llm_backend_user_choice.md` — `--llm-backend` flag)
- ADR-0039 (`llm_host_slot_semaphore.md`)
- ADR-0040 (`stderr_capture_fallback_chain.md` — codex OAuth 401 incident of 2026-06-14)
...
### Test Suite
- 807 tests passing, 0 failing, 1 ignored (G58 S1 stub)
- 2 tests more than v1.0.81 (805 → 807) with the addition of the 3 new subcommands
