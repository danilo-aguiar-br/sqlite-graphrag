...
- **Compliance opt-in** via flag `--strict-env-clear` (v1.1.8: product env não é o caminho de configuração). Modo estrito preserva apenas `PATH` e descarta as demais env vars; atende ambientes PCI-DSS, SOC2 e HIPAA que proíbem encaminhamento de credenciais via env vars
