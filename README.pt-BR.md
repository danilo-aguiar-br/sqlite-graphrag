...
> **Release atual: v1.2.8.** Contrato permanente em toda a linha 1.2.x: schema **v17** (a V017 abriu o vocabulário de `entity_type`; a v1.2.7 e anteriores publicaram v16), `DEFAULT_EMBEDDING_DIM=1024`, precedência de configuração **flag CLI > XDG `config set` > default** (env de produto `SQLITE_GRAPHRAG_*` **não** é lida no hot path), embedding e enrich **somente por OpenRouter REST**, releases manuais (sem GitHub Actions), owner no crates.io `danilo-aguiar-br`. O que cada release mudou está em [CHANGELOG.pt-BR.md](CHANGELOG.pt-BR.md) — este banner não é uma segunda cópia dele.
...
- O **contrato público é a CLI**. Os envelopes `--json` documentados em `docs/schemas/*.schema.json` permanecem estáveis em todas as versões v1.x.y. Consumidores que dependem apenas da CLI não são afetados por bumps minor ou patch.
- **Nenhuma variável de ambiente faz parte desse contrato.** O binário lê exatamente três variáveis de ambiente em runtime — `CLICOLOR_FORCE`, `NO_COLOR` e `XDG_RUNTIME_DIR` — e nenhuma delas é configuração de produto; env de produto **não é lida**. Revisões anteriores desta seção prometiam estabilidade para "as variáveis de ambiente listadas em `llms.txt` e `llms-full.txt`", o que contradizia a regra de env de produto proibida declarada adiante e no banner acima.
...
- Stderr carrega saída de tracing apenas sob `-v`/`-vv`/`-vvv` ou a chave XDG `log.level`
...
- `entity_type` aceita QUALQUER termo desde a v1.2.8; o rótulo que você envia é gravado como escrito e nunca é substituído por outro. 13 valores são RECOMENDADOS: `project`, `tool`, `person`, `file`, `concept`, `incident`, `decision`, `memory`, `dashboard`, `issue_tracker`, `organization`, `location`, `date`
...
- Um `entity_type` fora dos 13 valores recomendados é aceito, gravado como escrito e reportado no array `warnings` da resposta. Passe `--strict-entity-types` para restringir a escrita aos 13 e ser recusado com código de saída 1 em vez disso. Só a forma é imposta: trim, minúsculas, hífen vira underscore, e recusa quando vazio, só dígitos, com quebra de linha ou maior que 64 caracteres. Valores customizados de `relation` são aceitos desde v1.0.49.
...
- **GraphRAG está habilitado por padrão e roda automaticamente.** Cada subcomando auto-inicializa seu `graphrag.sqlite` se ele não existir — no caminho dado por `--db`, senão no `db.path` persistido, senão no diretório de **dados** XDG (`~/.local/share/sqlite-graphrag/graphrag.sqlite`), nunca no diretório de trabalho atual a menos que esse diretório de dados não possa ser resolvido. A extração de entidades/relacionamentos vem do transporte de embedding/LLM escolhido por `--llm-backend` ou de grafo curado (`--graph-stdin`, `--entities-file`). Não existe flag --extraction-backend: passá-la é recusado pelo clap com **exit 2** (`unexpected argument`).

### Extração automática (`--enable-ner`)
- Passe `--enable-ner` para ativar extração automática em `remember` e `ingest` (env de produto não é lida em runtime; v1.2.0)
...
- **`graphrag.sqlite` é criado no diretório de dados XDG por padrão** — `~/.local/share/sqlite-graphrag/graphrag.sqlite` (sobrescreva com `--db <caminho>` após o subcomando, ou persista via `config set db.path <caminho>`; o diretório de trabalho atual só é usado quando o diretório de dados não pode ser resolvido; product env `SQLITE_GRAPHRAG_DB_PATH` **não** é lida em runtime na v1.2.0)
...
```
- Sem `--db` (ou um `db.path` persistido via `config set`), todo comando CRUD resolve o banco do diretório de dados XDG, e **não** `./graphrag.sqlite`: o envelope reporta `db_path_source: "default"` e `db_path_resolved` apontando para `~/.local/share/sqlite-graphrag/graphrag.sqlite`. Product env `SQLITE_GRAPHRAG_DB_PATH` **não** é lida em runtime (v1.2.0)
...
# Ou persista como chave XDG (a flag CLI continua tendo precedência):
...
| `export` | `--namespace`, `--type`, `--include-deleted`, `--limit`, `--offset` | Exporta memórias como NDJSON para backup ou migração. Contrato de stream (GAP-SG-215, v1.2.8): a linha de registro carrega o registro e nada mais, a linha de sumário final carrega o único registro agent-surface do stream inteiro e nunca é moldada; `--select` e `--truncate-content` agem por registro, enquanto `--count-only`, `--sort`, `--dedupe-by`, `--max-items`, `--max-output-bytes` e `--filter` recusam com exit 2 antes da primeira linha |
...
| `graph entity-types` (v1.2.8) | Audita o vocabulário de tipos que o banco realmente contém — cada `type` com seu `count` e um sinalizador `canonical`, do mais frequente ao menos | `--namespace`, `--format json\|text` |
...
> **Removido na v1.2.8:** a família de subcomandos `pending` (`list` / `show <id>` / `cleanup`), enviada na v1.0.82 para a fila de checkpoint em 3 estágios do `remember` (GAP-001, ADR-0036), não existe mais — o binário responde `unrecognized subcommand`. A fila de retry de embedding não foi afetada e mantém sua própria superfície em `pending-embeddings` e `embedding`.
...
### Outras flags globais (v1.2.8)

Estas flags de topo não são transporte de embedding nem remodelagem de saída, e três delas mudam o que o binário RECUSA. Valem para todos os subcomandos.

| Flag | Descrição |
| --- | --- |
| `--use-active` | GAP-SG-207: aceita o banco alvo ambiente para um verbo que altera estado durável. Um subcomando que muta normalmente precisa nomear o alvo com `--db` e é recusado com **exit 2** quando não o faz; esta é a dispensa explícita, e o envelope registra que um humano a pediu |
| `--fail-on-degraded` | Falha em vez de degradar quando o embedding da consulta não pode ser produzido. Sem ela, `recall` e `hybrid-search` caem em ranking só por FTS5, levantam `vec_degraded` e ainda saem com `0` — um agente que parseia `.results` recebe em silêncio uma busca por palavra-chave onde pediu híbrida |
| `--wait-lock <SEGUNDOS>` | Espera até `SEGUNDOS` por um slot de concorrência livre antes de desistir (**exit 75**), com polling a cada 500 ms. Default 300s |
| `--llm-model <MODELO>` | v1.0.82 (GAP-003): modelo a invocar no backend escolhido. Prefira a flag; XDG `llm.model` é opcional |
| `--skip-embedding-on-failure` | v1.0.82 (GAP-005): persiste com embedding NULL quando todos os backends da cadeia falham. A memória fica em `pending_embeddings` para reprocessamento. Prefira a flag; XDG `llm.skip_embedding_on_failure` é opcional |

### Flags globais v1.2.2 — superfície de saída agent-native (GAP-SG-142)

Dez flags globais compõem a superfície agent-native: as duas guardas da subseção `Guardas da superfície` decidem quando um pedido de remodelagem é recusado, e as **oito** abaixo remodelam o envelope JSON em um único ponto, para que um agente pare de carregar um filtro `jaq` no prompt só para ler um campo. Elas valem para **todos** os subcomandos e compõem em ordem fixa: **filter → sort → dedupe → max-items → select → count-only → truncate-content → max-output-bytes**.
...
#### Guardas da superfície (v1.2.6 / v1.2.8)
Duas outras flags globais não remodelam nada; elas ampliam o que as flags de remodelagem podem aceitar, e sem elas a recusa é o default.

| Flag | Descrição |
| --- | --- |
| `--filter-scope <ESCOPO>` | GAP-SG-201: declara o que `--filter` pode observar. Omitida, um predicado sobre uma página que a consulta já truncou é recusado com **exit 2**, porque a resposta descreveria um conjunto que o predicado nunca viu. `page` aceita a leitura mais estreita e a registra em `count_scope`; `universe` declara o requisito explicitamente. Um limite top-k nunca é recusado: o k É a resposta |
| `--allow-unknown-keys` | GAP-SG-202: aceita uma chave de `--select` / `--filter` / `--sort` / `--dedupe-by` que este envelope não carrega em lugar nenhum. Sem ela tal chave é recusada com **exit 2**, porque uma chave irresolúvel produz resposta vazia indistinguível de dado ausente — um typo leria como "a memória não existe" |
...
A recusa é **declarativa, não emergente**. Sem a flag, um caminho de stdin só falha quando a leitura é tentada — imediatamente em TTY, depois do deadline nos demais casos. Com ela, `--body-stdin`, `--graph-stdin`, `remember-batch` e todo outro leitor de stdin falham de antemão com **exit 1** (`AppError::Validation`), mesmo com um pipe conectado que teria fornecido dados. É exatamente esse o objetivo: automação desassistida deve falhar rápido e alto, em vez de travar esperando um humano que não está lá.
...
| `1..=1000000` (limite de listagem) | `export --limit`, `pending-embeddings --limit`, `embedding --limit` |
...
### Referência completa das chaves `config set` (70 chaves, v1.2.8)
...
| Chave | Default | Finalidade |
| --- | --- | --- |
| `db.path` | `(nenhum)` | Banco padrão. Sobrescrito por `--db <PATH>` depois do subcomando. Sem nenhum dos dois, o diretório de dados XDG `~/.local/share/sqlite-graphrag/graphrag.sqlite` |
...
| `enrich.entity_description.grounding_threshold` | `0.30` | Score mínimo de ancoragem para a descrição ser mantida |
| `enrich.entity_description.corpus_top_k` | `8` | Memórias amostradas como evidência por entidade |
| `enrich.entity_description.min_corpus_chars` | `40` | Tamanho mínimo de evidência antes de chamar o LLM; abaixo dele a entidade é pulada, nunca descrita |
| `enrich.entity_description.neighbour_top_k` | `12` | Relações tipadas do grafo amostradas como evidência por entidade |
| `enrich.entity_description.snippet_chars` | `2000` | Caracteres por trecho de evidência |
| `enrich.entity_description.quality_sample` | `50` | Tamanho da amostra por trás de `quality_pct` no `enrich --status` |
| `enrich.entity_type.allowed_types` | `(nenhum)` | Vocabulário de tipos de entidade, separado por vírgula, que o `entity-type-validate` aceita. Vazio significa o conjunto canônico. Sobrescrito por `--allowed-types` |
| `enrich.entity_type.on_unknown_type` | `keep` | O que o `entity-type-validate` faz com um rótulo fora desse vocabulário: `keep` grava como escrito (comportamento da v1.2.8), `fallback` grava o aceito mais próximo e preserva o bruto na descrição, `strict` recusa com exit 1. Sobrescrito por `--on-unknown-type` |
| `enrich.entity_type_validate.corpus_top_k` | `8` | Corpos de memória ligados mostrados ao `entity-type-validate` como evidência |
| `enrich.entity_type_validate.min_corpus_chars` | `40` | Abaixo disso a entidade não tem evidência e a operação se abstém sem gastar token |
| `enrich.entity_type_validate.neighbour_top_k` | `12` | Relações tipadas do grafo mostradas ao `entity-type-validate` como evidência |
| `enrich.entity_type_validate.snippet_chars` | `2000` | Caracteres por trecho de evidência no `entity-type-validate` |
...
- Modo de baixa memória: passe `--low-memory` (ou persista `config set ingest.low_memory true`) para forçar ingest single-threaded. Equivale a `--ingest-parallelism 1` e sobrescreve qualquer valor explícito, ao custo de 3-4x mais tempo de relógio.
...
- O comportamento padrão cria ou abre `graphrag.sqlite` no diretório de dados XDG, e não no diretório atual — passe `--db <PATH>` depois do subcomando quando o banco precisa ficar junto do projeto
...
- Chamadas de embedding falhando com exit 11 normalmente indicam CLI LLM ausente, sem autenticação (OAuth obrigatório) ou timeout — aumente `--openrouter-timeout <SEGUNDOS>` ou a chave XDG `embedding.timeout_secs` (padrão 300) em links lentos
