...
> **Release atual: v1.2.0 — DEFAULT_EMBEDDING_DIM=1024; fila enrich com namespace + `--list-skipped`/`--requeue-skipped`; flag > XDG > default (sem product env); tracing-appender + testes herméticos IsolatedEnv; gate offline `scripts/e2e_offline_v120.sh`. Schema main permanece em v16. Releases só manuais (sem GitHub Actions). Owner no crates.io `danilo-aguiar-br`.**
