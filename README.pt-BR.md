...
> **Release atual: v1.1.04 — fecha os dois gaps estruturais rastreados em `gaps.md` após a v1.1.03: o panic de nested Tokio runtime no `deep-research` (GAP-001) e o enriquecimento `entity-connect` não-convergente (GAP-002). O schema avança da versão 15 para a 16 com uma migração numerada (V016 `entity_connect_seen`). Nota: o nome oficial do release é v1.1.04; o manifesto do crate carrega `version = "1.1.4"` porque o parser SemVer rejeita zero à esquerda no componente patch.** Release anterior v1.1.03 — fecha os seis bugs que bloqueavam operadores catalogados em `gaps.md` mais o portão V8 de corpo excessivamente grande (Bug 1: o caminho de scan-enqueue do enrich insere candidatos em lote em uma única transação em vez de linha a linha sob o write lock do WAL; Bug 2: `reclassify-relation --literal-to <RELATION>` migra as 61 357 arestas legadas com underscore para a forma canônica com hífen; Bug 3: `merge-entities --cross-namespace` (opt-in, default mesmo-namespace) resolve `--ids`/`--into-id` através de todos os namespaces; Bug 4: o sidecar do enrich ganha uma coluna `claimed_at`, claims stale em `processing` são resetadas no startup, um handler de SIGTERM faz cleanup graceful antes do exit 19, mais `enrich --reset-stale-claims` e `--stale-claim-secs <N>`; Bug 5: apenas documentação — a ajuda do `enrich --status` clarifica `scan_backlog` vs `queue_pending` vs cooldown vs deadlock; Bug 6: `re-embed --target chunks` troca para `LEFT JOIN memories` para que chunks de mães soft-deleted atinjam 100% de cobertura). O novo subcomando `split-body` divide memórias com mais de 25 000 caracteres em filhas (rode `enrich --operation re-embed --target memories` depois). O schema permanece em v15 (a fila sidecar ganha uma coluna `claimed_at` via ALTER idempotente), sem migração. O manifest do crate carrega `version = "1.1.3"` porque o SemVer rejeita zero à esquerda no componente patch.** Release anterior v1.1.02 — fecha os dois gaps residuais deixados após v1.1.01 (o argumento depreciado `--gliner-variant` (no-op) é totalmente removido — clap o rejeita com exit 2; o teto de tokens de embedding vira o erro tipado exit-6 `AppError::TooManyTokens { tokens, limit }`; um teste de regressão guarda o dispatch de re-embed de entidades; e `enrich --prune-dead-entity-orphans` remove linhas dead-letter entity-keyed da fila sidecar). O schema permanece em v15, sem migração.
