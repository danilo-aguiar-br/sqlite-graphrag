...
> **Release atual: v1.1.05 — fecha os cinco bugs operacionais do relato de incidente deep-research sobre o sujeito "danilo" (2026-07-08; ver `gaps.md`). Sem migração de schema. Nome oficial do release: v1.1.05; o manifesto do crate carrega `version = "1.1.5"` porque o parser SemVer rejeita zero à esquerda no componente patch.** Bug 1: `deep-research` com query de palavra única expande em sub-queries multi-aspecto (`source: "aspect"`); Bug 2: `--output PATH` grava o envelope via algoritmo atomwrite (tempfile → fsync → rename) com ack `blake3` no stdout + `--quiet`/`-q` global e contrato stdout-JSON / stderr-logs; Bug 3: `graph traverse --from <nome-curto>` com sugestões Jaro-Winkler e `--fuzzy` para auto-resolução; Bug 4: `merge-entities` rejeita self-ref (`--ids` contendo `--into-id`) **antes** de qualquer trabalho no DB; Bug 5: `link --from-id`/`--to-id` + rejeição de nomes só de dígitos. Release anterior v1.1.04 — fecha os dois gaps estruturais rastreados em `gaps.md` após a v1.1.03: o panic de nested Tokio runtime no `deep-research` (GAP-001) e o enriquecimento `entity-connect` não-convergente (GAP-002). O schema avança da versão 15 para a 16 com uma migração numerada (V016 `entity_connect_seen`). Nota: o nome oficial do release é v1.1.04; o manifesto do crate carrega `version = "1.1.4"` porque o parser SemVer rejeita zero à esquerda no componente patch.
