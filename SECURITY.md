...
- Bypass (emergencies only): `sqlite-graphrag config set spawn.skip_preflight 1` disables all 7 guards. Last-resort opt-out; bypassing reverts to direct Command::spawn() and inherits all 5 BUG classes. Product env is not the supported path in v1.1.8.
