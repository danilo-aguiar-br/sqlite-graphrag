#!/usr/bin/env bash
# GAP-CLI / Wave 0: offline E2E gate for sqlite-graphrag v1.2.0
...
# GAP-SG-96: list/requeue-skipped surface exists and is read-only offline
check list_skipped 0 "$BIN" enrich --db "$DB" --operation entity-descriptions --list-skipped --quiet
check requeue_skipped 0 "$BIN" enrich --db "$DB" --operation entity-descriptions --requeue-skipped --quiet
# GAP-SG-93: effective embedding.dim default is 1024
EFF=$("$BIN" config list --effective --json 2>/dev/null || true)
if echo "$EFF" | grep -q '"embedding.dim": "1024"'; then
  echo "PASS effective_dim_1024"
  pass=$((pass+1))
else
  echo "FAIL effective_dim_1024 in: $EFF"
  fail=$((fail+1))
fi
...
if echo "$HELP_ENRICH" | grep -E 'list-skipped|requeue-skipped' >/dev/null; then
  echo "PASS help_skipped_flags"
  pass=$((pass+1))
else
  echo "FAIL help_skipped_flags missing from enrich --help"
  fail=$((fail+1))
fi
