...
| `schema` | (none), `--name <ID>` | Machine-readable catalog of all **75** JSON contracts (v1.2.2). Bare `schema` emits NDJSON, one `{"id","invoke"}` per line, where `invoke` is the ready-to-copy command; `--name <ID>` emits that contract's JSON Schema document. Unknown `<ID>` exits **4**. `$schema` documents are exempt from the agent-native output surface, so any global flag chains safely |
...
| Command | Arguments | Description |
| --- | --- | --- |
| `pending` | `list`, `show <id>`, `cleanup`, `--status pending\|in_progress\|done\|abandoned`, `--limit`, `--json` | Inspect and process the three-stage `remember` checkpoint queue (GAP-001, ADR-0036); `cleanup` removes terminal-state rows |
...
Eight top-level flags reshape the JSON envelope at a single point, so an agent stops carrying a `jaq` filter in its prompt just to read one field. They apply to **every** subcommand and compose in a fixed order: **filter → sort → dedupe → max-items → select → count-only → truncate-content → max-output-bytes**.
...
### Complete `config set` key reference (63 keys, v1.2.5)
...
| Key | Default | Purpose |
| --- | --- | --- |
| `db.path` | `(none)` | Default database file. Overridden by `--db <PATH>` after the subcommand. Without either, `./graphrag.sqlite` |
...
| `enrich.entity_description.grounding_threshold` | `0.12` | Minimum grounding score for a description to be kept |
| `enrich.entity_description.corpus_top_k` | `5` | Memories sampled as evidence per entity |
| `enrich.entity_description.min_corpus_chars` | `40` | Minimum evidence length before the LLM is called |
| `enrich.entity_description.snippet_chars` | `400` | Characters per evidence snippet |
