...
> **Current release: v1.1.05 — closes the five operator-blocking bugs from the 2026-07-08 deep-research incident report on subject "danilo" (see `gaps.md`). No schema migration (schema stays at v16 from v1.1.04). Bug 1: `deep-research` single-token queries expand to multi-aspect sub-queries (`source: "aspect"`, EN/PT facets; manual via `--sub-query-strategy manual --sub-queries-file`). Bug 2: `deep-research --output PATH` atomic write (atomwrite) plus stdout ack (`written`/`bytes`/`blake3`) and global `--quiet`/`-q`. Bug 3: `graph traverse --fuzzy` plus NotFound suggestions (rapidfuzz Jaro-Winkler + prefix). Bug 4: `merge-entities` rejects self-ref if `--into-id` is in `--ids` (or names) BEFORE any DB work. Bug 5: `link --from-id`/`--to-id`; pure digit names rejected so `--create-missing` cannot create ghost numeric entities. Note: the official release name is v1.1.05; the crate manifest carries `version = "1.1.5"` because the SemVer parser rejects a leading zero in the patch component.** Previous release v1.1.04 — closes the two structural gaps tracked in `gaps.md` after v1.1.03: the `deep-research` nested-Tokio-runtime panic (GAP-001) and the non-convergent `entity-connect` enrichment (GAP-002). The schema advances from version 15 to 16 with a numbered migration (V016 `entity_connect_seen`). Note: the official release name is v1.1.04; the crate manifest carries `version = "1.1.4"` because the SemVer parser rejects a leading zero in the patch component. Previous release v1.1.03 — closes the six operator-blocking bugs catalogued in `gaps.md` plus the V8 oversized-body gate (Bug 1: the enrich scan-enqueue path batches candidate inserts in a single transaction instead of row-by-row under the WAL write lock; Bug 2: `reclassify-relation --literal-to <RELATION>` migrates the 61 357 legacy underscore edges to canonical hyphen form; Bug 3: `merge-entities --cross-namespace` (opt-in, default same-namespace) resolves `--ids`/`--into-id` across all namespaces; Bug 4: the enrich sidecar gains a `claimed_at` column, stale `processing` claims reset on startup, a SIGTERM handler performs graceful cleanup before exit 19, plus `enrich --reset-stale-claims` and `--stale-claim-secs <N>`; Bug 5: docs-only — `enrich --status` help clarifies `scan_backlog` vs `queue_pending` vs cooldown vs deadlock; Bug 6: `re-embed --target chunks` switches to `LEFT JOIN memories` so chunks of soft-deleted mothers reach 100% coverage). New `split-body` subcommand divides memories over 25 000 characters into daughters (run `enrich --operation re-embed --target memories` afterwards). The schema stays at v15 (the sidecar queue gains a `claimed_at` column via idempotent ALTER), no migration. The crate manifest carries `version = "1.1.3"` because SemVer rejects a leading zero in the patch component. Previous release v1.1.02 — closes the two residual gaps left after v1.1.01 (the deprecated `--gliner-variant` no-op argument is fully removed — clap rejects it with exit 2; the embedding token ceiling becomes the typed exit-6 error `AppError::TooManyTokens { tokens, limit }`; a regression test guards the re-embed entity dispatch; and `enrich --prune-dead-entity-orphans` prunes entity-keyed dead-letter rows). The schema stays at v15, no migration.
