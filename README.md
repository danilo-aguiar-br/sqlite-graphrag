...
> **Current release: v1.2.0 — DEFAULT_EMBEDDING_DIM=1024; enrich queue namespace + `--list-skipped`/`--requeue-skipped`; flag > XDG > default (no product env); tracing-appender + hermetic IsolatedEnv tests; offline gate `scripts/e2e_offline_v120.sh`. Schema main stays at v16. Manual releases only (no GitHub Actions). crates.io owner `danilo-aguiar-br`.**
